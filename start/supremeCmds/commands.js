...
async function playCommand(supreme, chatId, message, args) {
    try {
        const text = args.join(' ').trim();
        
        if (!text) return supreme.sendMessage(chatId, { 
        ...
        if (/youtu\.?be/.test(text)) {
            ...
            if (!id) return supreme.sendMessage(chatId, { 
        ...
        // Search YouTube for song name
            ...
            if (!searchResults.videos || searchResults.videos.length === 0) {
                return await supreme.sendMessage(chatId, { 
        ...
        const previewMsg = await supreme.sendMessage(chatId, {
            image: { url: thumbnail },
            caption: `🎵 *${title}*\n\n⌛ Downloading audio... Please wait...`
        }, { quoted: message });

        // Add loading reaction
        await supreme.sendMessage(chatId, { react: { text: '⏳', key: message.key } });
        ...
        const songmsg = await supreme.sendMessage(chatId, { 
            text: formatMenu 
        }, { quoted: message });
        ...
        const selectionHandler = async (msgUpdate) => {
            try {
                ...
                if (
                    ...
                    supreme.ev.off('messages.upsert', selectionHandler);
                    
                    await supreme.sendMessage(chatId, { react: { text: "⬇️", key: mp3msg.key } });
                    ...
                    // Success reaction
                    await supreme.sendMessage(chatId, { react: { text: '✅', key: mp3msg.key } });
                }
            } catch (error) {
        ...
        supreme.ev.on('messages.upsert', selectionHandler);
...
async function takeCommand(supreme, chatId, message, args) {
    try {
        // Check if message is a reply to a sticker
        const quotedMessage = message.message?.extendedTextMessage?.contextInfo?.quotedMessage;
        if (!quotedMessage?.stickerMessage) {
            await supreme.sendMessage(chatId, { text: '❌ Reply to a sticker with .take <packname>' });
            return;
        }
        ...
        try {
            ...
            const stickerBuffer = await downloadMediaMessage(
                ...
                {
                    logger: console,
                    reuploadRequest: supreme.updateMediaMessage
                }
            );

            if (!stickerBuffer) {
                await supreme.sendMessage(chatId, { text: '❌ Failed to download sticker' });
                return;
            }
            ...
            await supreme.sendMessage(chatId, { text: '❌ Error processing sticker' });
        }

    } catch (error) {
        console.error('Error in take command:', error);
        await supreme.sendMessage(chatId, { text: '❌ Error processing command' });
    }
}

async function videoCommand(supreme, chatId, message) {
    try {
        ...
        const processingMsg = await supreme.sendMessage(chatId, { 
        ...
        const infoMsg = await supreme.sendMessage(chatId, { text: caption }, { quoted: message });
        ...
        await supreme.sendMessage(chatId, { text: errorMessage }, { quoted: message });
        await supreme.sendMessage(chatId, { react: { text: '❌', key: message.key } });
    }
}
async function ytplayCommand(supreme, chatId, query, message) {
    try {
        if (!query) {
            return await supreme.sendMessage(chatId, {
...
async function InstagramCommand(supreme, chatId, message) {
    try {
        const text = message.message?.conversation || message.message?.extendedTextMessage?.text;
        const args = text.split(' ').slice(1); // Remove command prefix
...
async function handleMediafireDownload(supreme, chatId, message) {
    try {
        const text = message.message?.conversation || message.message?.extendedTextMessage?.text;
        const args = text.split(' ').slice(1); // Remove command prefix
        ...
        if (error.message.includes('API returned an error')) {
            ...
            errorMessage += 'The file was not found. It may have been deleted or the URL is incorrect.';
        } else {
            errorMessage += 'Please check the URL and try again.';
        }
